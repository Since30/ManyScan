...
const StarRating: React.FC<StarRatingProps> = ({rating, animeId}) => {
    const maxRating = 10;
    const ratingPercent = `${
        Math.round(((rating / maxRating) + Number.EPSILON) * 100)}%`;
    return (
        <svg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 24 24' className='w-8 h-8 m-auto'>
            <defs>
                <mask id={animeId}>
                    <rect
                        x='-3'
